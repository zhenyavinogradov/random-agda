...
--applyAllPredVS : ∀ {ρs} → AllPredVS ρs → Env ρs → Set
--applyAllPredVS ε ε = ⊤
--applyAllPredVS (P ∷ Ps) (value ∷ env) = applyPredV P value × applyAllPredVS Ps env

getAllPredVS : ∀ {τs τ} → AllPredVS τs → Has τs τ → Value τ → Set
--getAllPredVS (P ∷ Ps) here value = applyPredVS P (inl value)
...
--NormValueF Fin-τ value = applyPredVS Fin-τ (inl value)
...
--NormValue-πF Fin-τs (#tuple vs) = applyAllPredVS Fin-τs vs
...
data NormValue-σF {τs} (Fin-τs : AllPredVS τs) : Value (σ τs) → Set where
  --normInj : ∀ {τ} → (i : Has τs τ) → {v : Value τ} → applyPredV (Fin-τs τ i) v → NormValue-σF Fin-τs (#inj i v)
...
mutual
  ...
  --GoodVSPred (ρs ⇒ τ) = mkPredVS (GoodVSF (NormValue-⇒F (AllGoodVSPred ρs) (GoodVSPred τ)))
  --GoodVSPred (σ τs) = mkPredVS (GoodVSF (NormValue-σF (AllGoodVSPred τs)))
  --GoodVSPred (π τs) = mkPredVS (GoodVSF (NormValue-πF (AllGoodVSPred τs)))
  --GoodVSPred nat = mkPredVS (GoodVSF NormValue-natF)
  ...
  {-
  AllGoodVSPred' : (τs : List Type) → AllPredVS' τs
  AllGoodVSPred' (τ ∷ τs) _ here = GoodVSPred τ
  AllGoodVSPred' (τ ∷ τs) _ (there i) = AllGoodVSPred' τs _ i
  -}
...
--NormValue {τ} = NormValueF (GoodVSPred τ)
NormValue {τ} = NormValue' τ

{-
fin* : ∀ {τ v} → NormValue {τ} v → GoodVS (inl v)
fin* norm-value = fin norm-value
fin* {ρs ⇒ τ} norm = norm
fin* {σ τs} norm = norm
fin* {π τs} norm = norm
fin* {nat} norm = norm
...
{-
nex* : ∀ {τ s} → GoodVS {τ} (step s) → GoodVS (inr s)
--nex* {ρs ⇒ τ} = nex
--nex* {σ τs} = nex
--nex* {π τs} = nex
--nex* {nat} = nex
...
--goodNormVS (normInl norm-value) = norm-value
...
--normApplyCase {v = #inj {τs} {τ} i v} norm-env (normInj _ norm-v) good-terms = goodNormClosure (({!norm-v!} , norm-env) & get2 good-terms i) ∷ ε
